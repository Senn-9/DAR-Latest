...
const inputCls =
  "w-full px-3 py-2 text-sm text-gray-900 border border-gray-200 rounded-lg bg-white focus:outline-none focus:ring-2 focus:ring-fuchsia-500 focus:border-transparent transition placeholder:text-gray-400";

export default function POPARPOProcessModal({
  visible,
  po,
  currentUser,
  onClose,
  onSubmit,
}: POPARPOProcessModalProps) {
  ...
  useEffect(() => {
    if (visible) {
      ...
      // Pre-fill approver name if available
      if (currentUser?.fullname) {
        setApprovedBy(currentUser.fullname);
      }
    } else {
      setRemarks("");
      setApprovedBy(currentUser?.fullname || "");
      setParpoDate(new Date().toISOString().split("T")[0]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      // Combine PARPO info with remarks
      const fullRemarks = `PARPO Date: ${parpoDate} | Approved By: ${approvedBy || "N/A"}\n${remarks}`;
      ...
      console.error("PARPO processing error:", error);
    } finally {
      setProcessing(false);
    }
  };
    ...
    <>
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <div className="relative z-10 bg-white rounded-2xl shadow-2xl w-full max-w-lg flex flex-col overflow-hidden">
          ...
          {/* ── HEADER ── */}
          <div className="bg-gradient-to-r from-fuchsia-600 to-fuchsia-700 px-6 py-5 flex items-center justify-between text-white">
            <div>
              <h2 className="text-lg font-bold">PARPO Processing</h2>
              <p className="text-fuchsia-100 text-sm mt-0.5 font-mono">{po.po_no ?? "Unknown PO"}</p>
            </div>
            <button onClick={onClose} className="hover:bg-fuchsia-500/50 p-2 rounded-lg transition-colors">
              <RiCloseLine size={22} />
            </button>
          </div>
          ...
          <div className="px-6 py-6 space-y-5 overflow-y-auto max-h-[70vh]">
            ...
            <div className="grid grid-cols-2 gap-3 text-sm">
              ...
              <div className="p-3 bg-gray-50 rounded-lg">
                ...
                <p className="font-semibold text-fuchsia-700">{fmtMoney(po.total_amount)}</p>
              </div>
              <div className="p-3 bg-gray-50 rounded-lg">
                <p className="text-xs text-gray-500">Current Status</p>
                <p className="font-semibold text-fuchsia-700">PO (PARPO)</p>
              </div>
            </div>

            {/* Info Banner */}
            <div className="flex items-start gap-3 px-4 py-3 bg-fuchsia-50 border border-fuchsia-200 rounded-lg">
              <div className="w-5 h-5 rounded-full bg-fuchsia-200 flex items-center justify-center flex-shrink-0 mt-0.5">
                <span className="text-xs font-bold text-fuchsia-700">i</span>
              </div>
              <div>
                <p className="text-sm font-semibold text-fuchsia-900">PARPO Approval Required</p>
                <p className="text-xs text-fuchsia-700 mt-1">
                  This PO requires PARPO ( Provincial Accountant's Review and Processing Office) approval before it can be served.
                </p>
              </div>
            </div>

            {/* PARPO Date and Approved By */}
            ...
            <div>
              ...
              <textarea
                ...
                rows={4}
                placeholder="Add remarks about PARPO approval..."
                value={remarks}
          ...
          <div className="px-6 py-4 bg-gray-50 border-t border-gray-100 flex items-center justify-between">
            ...
            <button
              ...
              disabled={processing}
              className="px-5 py-2 text-sm font-semibold text-white bg-fuchsia-600 hover:bg-fuchsia-700 disabled:opacity-50 disabled:cursor-not-allowed rounded-lg shadow-sm transition-colors flex items-center gap-2"
            >
              {processing ? (
                <>
