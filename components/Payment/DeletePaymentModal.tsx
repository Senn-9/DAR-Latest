...
import { RiDeleteBinLine, RiCloseLine, RiFileListLine, RiCheckboxCircleLine, RiReceiptLine, RiMoneyDollarCircleLine } from "react-icons/ri";
import { fetchPaymentDeletePreview, deletePaymentDeep, type DeletePaymentPreview } from "@/utils/supabase/delivery";
...
export default function DeletePaymentModal({
  ...
  const [preview, setPreview] = useState<DeletePaymentPreview | null>(null);
  ...
  useEffect(() => {
    ...
    fetchPaymentDeletePreview(deliveryId)
      .then(setPreview)
      .catch((e) => {
  ...
  const handleDelete = async () => {
    ...
    setSaving(true);
    try {
      await deletePaymentDeep(deliveryId);
      onDeleted(String(deliveryId));
      onClose();
    } catch (e: any) {
      console.error("Failed to delete payment:", e);
      alert(e?.message ?? "Could not delete payment record.");
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden">
        ...
        <div className="p-6 space-y-4">
          <div className="bg-red-50 border border-red-200 rounded-xl px-4 py-3 flex items-start gap-3">
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-red-700 mt-1 leading-5">
                This will delete the payment record and its linked voucher, ORS, DV documents and payment remarks. This action cannot be undone.
              </p>
            </div>
          </div>
          ...
          <div className="bg-white border border-gray-200 rounded-xl px-4 py-3">
            ...
            <p className="text-xs text-gray-500 mt-1 leading-5">
              Deletion is scoped by primary keys. A Payment delete removes Payment-owned documents linked via delivery_id.
            </p>
            <div className="mt-2 rounded-xl bg-gray-50 px-3 py-2.5 border border-gray-100">
              ...
              <p className="text-xs text-gray-500 mt-1 leading-5">
                This payment record and its voucher/ORS/DV document rows and payment remarks.
              </p>
            </div>
            <div className="mt-2 rounded-xl bg-amber-50 px-3 py-2.5 border border-amber-200">
              ...
              <p className="text-xs text-amber-700 mt-1 leading-5">
                The connected delivery record, PO (and its items/remarks) and any linked PR records. Only this payment phase is removed.
              </p>
            </div>
          </div>
          ...
          {loadingPreview ? (
            ...
            <div className="grid grid-cols-4 gap-2">
              <div className="bg-yellow-50 border border-yellow-200 rounded-xl py-2.5 px-2 text-center">
                <RiMoneyDollarCircleLine size={16} className="text-yellow-700 mx-auto" />
                <p className="text-sm font-bold mt-1 text-yellow-700">{preview?.voucherCount ?? 0}</p>
                <p className="text-xs font-bold text-yellow-600 mt-0.5">Voucher</p>
              </div>
              <div className="bg-purple-50 border border-purple-200 rounded-xl py-2.5 px-2 text-center">
                <RiFileListLine size={16} className="text-purple-700 mx-auto" />
                <p className="text-sm font-bold mt-1 text-purple-700">{preview?.orsCount ?? 0}</p>
                <p className="text-xs font-bold text-purple-600 mt-0.5">ORS</p>
              </div>
        ...
        <div className="px-6 py-4 bg-gray-50 border-t border-gray-100 flex gap-2">
          ...
          <button
            onClick={() => {
              if (window.confirm("Delete payment record permanently? This cannot be undone. Continue?")) {
                handleDelete();
              }
            }}
